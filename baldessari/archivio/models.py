...
class Project(models.Model):
    ...
    impresa = models.CharField(max_length = 200, null = True, blank = True)
                                              
    descrizione_prog = models.TextField(null = True, blank = True)
...
class Document(models.Model):
    ...
    archivio = models.CharField(max_length = 3, choices = ARCHIVIO_CHOICE, null = True, blank = True)
...
class BioDetail(models.Model):
    ...
    testo = models.CharField(max_length = 10000,blank = True, null=True)
...
def saveImageAdress(sender, instance, created, **kwargs):
    ...
    if instance.image != "img_documents/noimage.jpg":
        ...
        shutil.copyfile(os.path.abspath(settings.MEDIA_ROOT+instance.image.url.replace("media/","")),path )


        thumbpath = "uploaded/thumbs/"+instance.archivio+"/"+proj+"/"+os.path.basename(instance.image.name)
        instance.thumbAdress=thumbpath
...
def saveLetterAdress(sender, instance, created, **kwargs):
    # figure out warranty end date

    post_save.disconnect(saveLetterAdress, sender=Letter)
    ...
    if instance.image != "img_documents/noimage.jpg":
        ...
        instance.thumbAdress="uploaded/thumbs/"+instance.archivio+"/"+proj+"/"+os.path.basename(instance.image.name)
        instance.save()
        
    post_save.connect(saveLetterAdress, sender=Letter)
