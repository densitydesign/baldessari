...
def viewproj(requestion, projsigle):
    ...
    casva = related_documents.filter(archivio="CASVA").count() | related_documents.filter(archivio="CAS").count()
    mart = related_documents.filter(archivio="MART").count() | related_documents.filter(archivio="MAR").count()
    carteggi = Letter.objects.filter(project=project)
    cart_polimi = carteggi.filter(archivio="ALB").count() | carteggi.filter(archivio=None).count()
    cart_casva = carteggi.filter(archivio="CASVA").count() | carteggi.filter(archivio="CAS").count()
    cart_mart = carteggi.filter(archivio="MAR").count() | carteggi.filter(archivio="MART").count()
    ...
    params = {
            ...
            'cart_polimi': cart_polimi,
            'cart_casva': cart_casva,
            'cart_mart': cart_mart,
